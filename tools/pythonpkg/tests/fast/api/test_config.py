...
class TestDBConfig(object):
    ...
    def test_multiple_options(self, duckdb_cursor, pandas):
        ...
        con = duckdb.connect(
            ":memory:",
            config={"default_null_order": "nulls_last", "default_order": "desc"},
        )
        result = con.execute("select * from df order by a").fetchall()
    ...
    def test_user_agent_default(self, duckdb_cursor):
        ...
        assert (
            regex.match(con_regular.sql("PRAGMA user_agent").fetchone()[0]) is not None
        )
    ...
    def test_user_agent_custom(self, duckdb_cursor):
        con_regular = duckdb.connect(
            ":memory:", config={"custom_user_agent": "CUSTOM_STRING"}
        )
