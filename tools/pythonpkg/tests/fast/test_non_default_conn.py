...
class TestNonDefaultConn(object):
    ...
    def test_query(self, duckdb_cursor):
        ...
        assert duckdb_cursor.query("select count(*) from t").execute().fetchall()[
            0
        ] == (1,)
        assert duckdb_cursor.from_query("select count(*) from t").execute().fetchall()[
            0
        ] == (1,)

    def test_from_csv(self, duckdb_cursor):
        temp_file_name = os.path.join(
            tempfile.mkdtemp(), next(tempfile._get_candidate_names())
        )
        duckdb_cursor.execute("create table t (a integer)")
        duckdb_cursor.execute("insert into t values (1)")
        ...
        assert rel.query(
            "t_2", "select count(*) from t inner join t_2 on (a = i)"
        ).fetchall()[0] == (1,)
    ...
    def test_project_df(self, duckdb_cursor):
        ...
        assert rel.query("t_2", "select * from t inner join t_2 on (a = i)").fetchall()[
            0
        ] == (1, 1)
    ...
    def test_query_df(self, duckdb_cursor):
        ...
        rel = duckdb.query_df(
            test_df,
            "t_2",
            "select * from t inner join t_2 on (a = i)",
