...
class TestArrowLargeOffsets(object):
    ...
    def test_large_lists(self, duckdb_cursor):
        ...
        pa.Table.from_pydict(dict(col=ary))
        with pytest.raises(
            ...
            duckdb_cursor.sql("SELECT col FROM tbl").arrow()

        pa.Table.from_pydict(dict(col=ary.cast(pa.large_list(pa.uint8()))))
    ...
    def test_large_maps(self, duckdb_cursor):
        ...
        with pytest.raises(
            ...
            duckdb_cursor.sql("select map(col, col) from tbl").arrow()
