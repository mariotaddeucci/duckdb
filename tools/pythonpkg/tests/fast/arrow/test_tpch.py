...
def check_result(result, answers):
    for q_res in answers:
        ...
        if (
            cq_results == [""]
            and str(db_result) == "None"
            or str(db_result[0]) == "None"
        ):
            continue
...
class TestTPCHArrow(object):
    def test_tpch_arrow(self, duckdb_cursor):
        ...
        for i in range(1, 23):
            query = duckdb_conn.execute(
                "select query from tpch_queries() where query_nr=" + str(i)
            ).fetchone()[0]
            answers = (
                duckdb_conn.execute(
                    "select answer from tpch_answers() where scale_factor = 0.01 and query_nr="
                    + str(i)
                )
                .fetchone()[0]
                .split("\n")[1:]
    ...
    def test_tpch_arrow_01(self, duckdb_cursor):
        ...
        for i in range(1, 23):
            ...
            answers = (
                duckdb_conn.execute(
                    "select answer from tpch_answers() where scale_factor = 0.1 and query_nr="
                    + str(i)
                )
                .fetchone()[0]
                .split("\n")[1:]
    ...
    def test_tpch_arrow_batch(self, duckdb_cursor):
        ...
        for tpch_table in tpch_tables:
            duck_tbl = duckdb_conn.table(tpch_table)
            arrow_tables.append(
                pyarrow.Table.from_batches(duck_tbl.arrow().to_batches(10))
            )
            duck_arrow_table = duckdb_conn.from_arrow(arrow_tables[-1])
            duckdb_conn.execute("DROP TABLE " + tpch_table)
            duck_arrow_table.create(tpch_table)
